 ...
 * 
 * More precisely, the following function defines how the page document object
 * is manipulated according to the page-specific data (the 'data' variable
 * defined in the page data.js file) and according to the structure of the
...
function manipulate(root) {
	...
	//$('#load').append('<input type="file" id="fileLoad" />');
	if (window.File && window.FileReader && window.FileList && window.Blob) {
		// alert("Fine. The File APIs supported.");
	} else {
		alert("Ouch. The File APIs are not fully supported in this browser.");
	};
	if (!window.btoa || !window.atob) {
		alert("Ouch. This browser can't encode or decode base64.");
	};
	if (!window.localStorage) {
		alert("Ouch. The localstorage API is not fully supported in this browser.");
	};
};
